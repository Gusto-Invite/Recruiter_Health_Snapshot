...
// ── Funnel ─────────────────────────────────────────────────────────
// When a team has zero reqs matched in Open Reqs / Current Pipeline per Job
// (both sheets are scoped to Engineering-adjacent departments — Engineering,
// Design, Data, Security, Product — and don't yet track GTM/CX as their own
// department), show an honest explanation instead of an all-zero funnel that
// looks broken.
function renderFunnelEmpty(teamName) {
  const el = document.getElementById('funnelContainer');
  if (el) el.innerHTML = `<div style="padding:24px 4px;text-align:center;color:var(--text2);font-size:12px;font-style:italic">
    No reqs currently matched to ${teamName} recruiters in Current Pipeline per Job / Open Reqs — these Greenhouse sheets are scoped to Engineering-adjacent departments (Engineering, Design, Data, Security, Product) and don't yet include ${teamName}.
  </div>`;
  const dateEl = document.getElementById('funnelDate');
  if (dateEl) dateEl.textContent = 'no live data source for this team';
  const riskNote = document.querySelector('#funnelContainer')?.closest('.card')?.querySelector('.analysis-note.risk');
